...
public class tcas1 {
	...
	{
		if(Climb_Inhibit >0)
			return Up_Separation + MINSEP;//v2:			return Up_Separation + NOZCROSS;
		else 
	...
	{
	    return (Own_Tracked_Alt <= Other_Tracked_Alt);//v6
	}
	 ...
	 int alt_sep_test()
	{
	    ...
	    enabled = High_Confidence && (Own_Tracked_Alt_Rate <= OLEV);//v15:enabled = High_Confidence && (Own_Tracked_Alt_Rate <= OLEV) && (Cur_Vertical_Sep > MAXALTDIFF);
